...
GitRepository.prototype.exec = async function (...args) {
  console.log("git", ...args, this.path);
...
function getStats(lines, collectMessages) {
  ...
  if (collectMessages && lines) {
    message = new RegExp(`${COMMIT_START}(.*)${COMMIT_END}`, "m").exec(
      lines
    )[1];
    lines = lines.replace(
      new RegExp(`${COMMIT_START}.*${COMMIT_END}`, "m"),
      ""
    );
  }
  lines = lines.split("\n");
...
GitRepository.prototype.authors = async function (collectMessages, ...args) {
  const data = await this.exec(
    ...
    `--format=${BREAK_LINE}%at %aE %aN${
      collectMessages ? COMMIT_START + "%s" + COMMIT_END : ""
    }`,
    "--find-renames",
    "--no-renames",
    "--numstat",
  ...
  authors.forEach((lines) => {
    const {
      ...
      linesChanged,
      message,
    } = getStats(lines, collectMessages);
    ...
    .map((author) => {
      ...
      return {
        ...
        commitsPercent: Number(
          ((authorMap[author].commits * 100) / totalCommits).toFixed(1)
        ),
        filesChangedPercent: Number(
          ((authorMap[author].filesChanged * 100) / totalFilesChanged).toFixed(
            1
          )
        ),
        linesAddedPercent: Number(
          ((authorMap[author].linesAdded * 100) / totalLinesAdded).toFixed(1)
        ),
        linesDeletedPercent: Number(
          ((authorMap[author].linesDeleted * 100) / totalLinesDeleted).toFixed(
            1
          )
        ),
        linesChangedPercent: Number(
          ((authorMap[author].linesChanged * 100) / totalLinesChanged).toFixed(
            1
          )
        ),
        ...authorMap[author],
