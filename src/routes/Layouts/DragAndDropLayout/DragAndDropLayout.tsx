...
import { applyColumn } from "~/components";
...
export class DragAndDropLayout extends React.Component {
  _generateLayout = (rowsCount = 3) => {
    ...
    for (let i = 0; i < rowsCount; i++) {
      ...
      while (availableRow > 0) {
        const newCol =
          availableRow < TOTAL_ROWS ? availableRow : _.random(3, 9);
  ...
  _lastLayout = this._generateLayout();
    ...
    return (
      <React.Fragment>
        <Container fluid={fluid}>
          ...
          <p>
            <strong>React-Grid Layout</strong> is a grid layout system much like
            Packery or Gridster for React. Unlike those systems, it is
            responsive and supports breakpoints. These breakpoints can be
            provided in the same way as in Reactstrap&apos;s Grid system.
          </p>
        ...
        >
          ...
          >
            {_.chain(this.state.layouts)
              ...
              .map((layoutKey) => (
                <FloatCol
                  {...applyColumn(layoutKey, this.state.layouts)}
                  key={layoutKey}
                >
                  <Card>
